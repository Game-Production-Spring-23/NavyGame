...
export class GameScene extends Scene {
  ...
  Start(app, data) {
    ...
    this.lockPoint = app.screen.width;
  ...
  Tick(app, data) {
    ...
    if (this.goRight) {
      ...
      if (this.character.x <= this.characterRightBound) {
        ...
        // the character is at the limit
        if (this.backGroundContainer.x > this.leftMaxDistBack) {
          //this.characterLeftBound = app.screen.width / 3;
          this.backGroundContainer.x -= this.backGroundScrollSpeed;
        } else {
          // character movement -> allow to reach end of screen
          if (this.isLocked == true) {
            this.characterRightBound =
              this.lockPoint - (this.character.width * 2) / 3;
          } // end if
        } // end if
  ...
  Unlock(app) {
    if (this.internalLock) {
      ...
      this.isLocked = true;
      this.leftMaxDistBack =
        -app.screen.width * this.backGroundRatio + app.screen.width;
      this.leftMaxDistMid =
        -app.screen.width * this.midGroundRatio + app.screen.width;
      this.leftMaxDistFore =
        -app.screen.width * this.foreGroundRatio + app.screen.width;
    } // end if
  } // end Unlock

  // creates the character sprite and mechanics
  createCharacter(app, data, groundLevel) {
    this.characterTexture = PIXI.Texture.from(data.characters.player.images[0]);
  ...
  setScrollSpeedAndRatios(
    ...
    this.midGroundScrollSpeed =
      this.scrollSpeed * this.midGroundRatio * this.backGroundRatio;
    this.foreGroundScrollSpeed =
      this.scrollSpeed * this.foreGroundRatio * this.midGroundRatio;
